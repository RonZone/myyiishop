...
<div class="product-form">
    ...
    <?php if (!$model->isNewRecord) { ?>
        <div class="form-group">
            <?php
            foreach ($model->productImages as $iamge) {
                echo '<div>';
                echo '<a href="' . \Yii::$app->getUrlManger()->createUrl(['product/remove', 'id' => $image->id]) . '" title="' . Yii::t('app', 'Delete') . '" data-confirm="' . Yii::t('app', 'Are you sure you want to delete this item?'). '" data-method="post" data-pjax="0"><span class="glyphicon glyphicon-trash"></span></a><br>';
                if (strpos($image->thumb, 'http://') === null) {
                    $file = Yii::getAlias('@frontend/web' . $image->thumb); //获取别名
                ...
                echo Yii::t('app', 'ort Order') . ' <input style="width:50px" name="imageSort[' . $image->id . ']" value="' . $image->sort_order . '">';
                echo '</div>';
            }?>
        </div>
        <div class="form-group">
